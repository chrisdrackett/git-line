...
export default {
  ...
  message(output) {
    ...
    const serviceName = isGithub ? 'on Github' : ' on Web';

    return (
      output.author === 'Not Committed Yet'
        ? msg
        : msg + ` View <a target="_blank" class="git-line-github-link" rel="noopener noreferrer" href="${this.githubUrl}/${commit}/${output.hash}">${serviceName}</a>.`
    );
  }
